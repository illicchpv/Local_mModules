var mButton0V1 = (() => {
  const moduleName = 'mButton0V1';

  // эта часть отвечает за создание и работу порождаемых экземпляров объектов 
  const instances = {};
  const createInstance = (instanceName) => {
    if (instances[instanceName]) throw new Error(`mButton0V1.instance '${instanceName}' already exist!`);
    ...
    const ins = {
      ...
      render() {
        ...
        this.iEl.
          setAttribute('title', `mButton0V1.${this.iname} #${this.clickCounter}`);
